...
export interface FantasySliderRoom {
    id: string;
    room_code: string;
...
export function useFantasySliderRoom() {
    ...
    const room: FantasySliderRoom | null = convexRoom ? {
        id: convexRoom._id,
        ...
        card_count: convexRoom.card_count || 5,
        ...
        phase: (convexRoom.phase === "waiting" ? "waiting" :
            convexRoom.phase === "clue" ? "playing" :
                convexRoom.phase === "ended" ? "ended" : "waiting") as FantasySliderRoom["phase"],
    } : null;
    ...
    const joinRoom = useCallback(async (
        ...
        try {
            const result = await joinRoomMutation({
                ...
                playerAvatar: avatar,
                expectedGameMode: "mini_fantasy_slider",
            });
