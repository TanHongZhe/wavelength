...
export function useFlagRoom() {
    ...
    const room: FlagRoom | null = convexRoom ? {
        id: convexRoom._id,
        ...
        team_score: convexRoom.psychic_score || 0,
        phase: (convexRoom.phase === "waiting" ? "waiting" :
            (convexRoom.phase === "clue" || convexRoom.phase === "rules") ? "playing" :
                convexRoom.phase === "ended" ? "ended" : "waiting") as FlagRoom["phase"],
    } : null;
    ...
    const joinRoom = useCallback(async (
        ...
        try {
            const result = await joinRoomMutation({
                ...
                playerAvatar: avatar,
                expectedGameMode: "mini_flag_game",
            });
