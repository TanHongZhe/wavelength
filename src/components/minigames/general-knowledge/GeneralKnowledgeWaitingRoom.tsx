...
export function GeneralKnowledgeWaitingRoom({
    ...
    return (
        <div className="fixed inset-0 z-50 bg-background flex items-center justify-center p-4">
            ...
            >
                <div className="mb-4">
                    <h2 className="text-xl font-bold font-display mb-1 text-primary">General Knowledge Lobby</h2>
                ...
                <div className="mb-5">
                    <h3 className="font-display font-bold text-base mb-3 flex items-center justify-center gap-2">
                        <Users className="w-4 h-4" /> Players ({players.length}/6)
                    </h3>
                    <div className="grid grid-cols-2 gap-2">
                        {players.map((p) => (
                            ...
                            >
                                ...
                                <span className="font-medium truncate text-xs">{p.name} {p.role === 'host' && '👑'}</span>
                            </motion.div>
                        ))}
                        {/* Empty slots placeholders */}
                        {Array.from({ length: Math.max(0, 6 - players.length) }).map((_, i) => (
                            <div key={`empty-${i}`} className="p-2 rounded-lg border border-border border-dashed bg-muted/20 flex items-center justify-center gap-2 opacity-50">
